...
iCheck = 0
itrim = 0
while iRead < iLen/18 do
iRndNote = int(random:i(iMin,iMax))*18
iInArr [iRead] = iRndNote
...
iOutArr2 = iOutArr
	iWrite = iCheck
	while iWrite < lenarray(iOutArr2)/18 do
		iOutArr2 [iWrite] = (RndRp:i(iOutArr2,iMin,iMax))*18
		iWrite +=1
	od
	iwrite = 0
	while iwrite < iLen do
		iRndNote += 1
		iOutArr2 [iwrite] = iRndNote
		;print iRndNote
		iwrite += 1
	od
iRead +=1
...
iWavParts = 0
iArrNumber [] genarray iWavParts,iWavParts+180
iLenParts lenarray iArrNumber
iFXnumber = int(iLenParts/5)
iArrFX1 [] RndArrN iFXnumber,iWavParts/18,(iWavParts+180)/18
iArrFX2 [] sorta iArrFX1
;printarray iArrFX2,"%.f"
	iCnt = 0
	iRead = 0
	while iCnt <  iLenParts  do
...
iPchSh_byps = (int(random:i(0,100)) < 20 ) ? 0 : 1
iFrqSh_byps = (int(random:i(0,100)) < 25 ) ? 0 : 1
iRngMd_byps = (int(random:i(0,100)) < 70 ) ? 0 : 1
iResns_byps = (int(random:i(0,100)) < 80 ) ? 0 : 1
