...
opcode FrqChng, ii,Si
  SNoteIn,iChng xin
iRatio = iChng/440
iMidi ntom SNoteIn
iFrq mtof iMidi
iFrqOut = iFrq*iRatio
iMidiOut ftom iFrqOut
  xout iMidiOut,iFrqOut
endop
...
iSeqArr[] fillarray 1, 1, 2, 1, 3, 2, 4
...
schedule p4,0,iDur,SNotesArr,SseqArr,iShahedMidi,iIstMidi
endin
...
kTimeShort random 0.2, 0.9
kTimeLong random 0.91, 2.1
kTime = random:k(0,100) > 70 ? kTimeShort : kTimeLong
kNoteIndx = int(random:k(0,iNoteLen))
kNote = random:k(0,100) > 40 ? iShahedMidi : iNoteArr[kNoteIndx]
if kTime > 1.9 then
kNote = iIstMidi
endif
kDur random kTime*1.5, kTime*3
...
fprints "shahed3.txt","%.2f %.3f\n",mtof(iShahedMidi+24),0
fprints "shahed3.txt","%.2f %.3f\n",mtof(iShahedMidi+24),85
fprints "ist3.txt","%.2f %.3f\n",mtof(iIstMidi+24),0
fprints "ist3.txt","%.2f %.3f\n",mtof(iIstMidi+24),85
...
kTime init 1
kCount init 0
kMoses linseg 60, p3, 0
if metro(1/kTime) == 1 then
kTime random 1, 6
kDur random kTime*0.7, kTime*2
kFrqMoses = random:k(0,100) > kMoses ? 10 : 30
...
iOctaveArr[] fillarray 1, 2, 4
iOctaveIndx = int(random:i(0, 3))
iOctaveRnd = (random:i(0,100) > 25 ? 1 : iOctaveIndx)
iOctave = iOctaveArr[iOctaveRnd]
iFrq = iFrqIn*iOctave
iAtt = 1
iRel = 3
 aEnv	transegr	0,iRel,3, 1,p3,4,0, iAtt,-4,0
iFrqMoses = p5
print iFrqMoses
...
aInShortL chnget "sndsL"
aInShortR chnget "sndsR"
aInLongL chnget "sndlL"
aInLongR chnget "sndlR"
fout "recordShort.wav", 8, aInShortL,aInShortR
fout "recordLong.wav", 8, aInLongL,aInLongR
aSumL sum aInShortL,aInLongL
aSumR sum aInShortR,aInLongR
